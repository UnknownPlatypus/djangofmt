...
A directive djangofmt cannot honor is reported rather than skipped silently: [`invalid-ignore-comment`](rules/invalid-ignore-comment.md) for a malformed or misplaced one, [`unknown-ignore-code`](rules/unknown-ignore-code.md) for a code naming no rule.
