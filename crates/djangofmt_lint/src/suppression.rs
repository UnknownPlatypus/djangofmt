...
pub fn collect_ignore_comments<'s>(
    ...
    root.jinja_comments
        ...
        .filter_map(|body| {
            ...
            let Range { start, end } = TEMPLATE_COMMENT.enclosing_range(checker, body);
            Some(IgnoreComment {
...
fn leading_file_ignore_codes(source: &str) -> Option<Vec<&str>> {
    let body = TEMPLATE_COMMENT.leading_body(strip_bom(source).trim_start())?;
    match IgnoreDirective::parse(body)? {
